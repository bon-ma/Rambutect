...
import org.tensorflow.lite.examples.objectdetection.ObjectDetectorHelper.Companion.MODEL_YOLO
import org.tensorflow.lite.examples.objectdetection.ObjectDetectorHelper.Companion.MODEL_YOLO_FRUIT
...
class YoloDetector(
    ...
    init {
        ...
        val modelPath =
            when (currentModel) {
                MODEL_YOLO -> "yolo11n_float32.tflite"
                MODEL_YOLO_FRUIT -> "yolo11n_float32-fruit.tflite"
                else -> "yolo11n_float32.tflite"
            }

        val metadataPath =
            when (currentModel) {
                MODEL_YOLO -> "metadata.yaml"
                MODEL_YOLO_FRUIT -> "metadata-fruit.yaml"
                else -> "metadata.yaml"
            }
